...
fn calc_scenic_score(
    ...
    let left = (1..x)
        .rev()
        .take_while(|&tx| tree_grid[y][tx] < tree)
        .count()
        + 1;
    let right = ((x + 1)..(width - 1))
        .take_while(|&tx| tree_grid[y][tx] < tree)
        .count()
        + 1;
    let up = (1..y)
        .rev()
        .take_while(|&ty| tree_grid[ty][x] < tree)
        .count()
        + 1;
    let down = ((y + 1)..(height - 1))
        .take_while(|&ty| tree_grid[ty][x] < tree)
        .count()
        + 1;
...
pub fn solve() {
    let tree_grid: Vec<Vec<u8>> = iterate_file_lines("day8input.txt")
